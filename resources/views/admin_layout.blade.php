...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{{ asset('dashboard2') }}/vendors/styles/style.css">
     ...
     <link rel="stylesheet" type="text/css" href="{{ asset('backend') }}/css/halflings.css">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <div class="notification-list mx-h-350 customscroll">
                            ...
                            $orders=App\Recived_Orders::where('status','off')->orderBy('created_at', 'DESC')->limit(10)->get();
                                
                            @endphp
                            @foreach ($orders as $element)
                          
                                <li>
                                    <a href="{{ route('edit_order_value',['id'=>$element->id]) }}">
                                        @if (!empty($element->user_id))

                                        @php
                                            $userData=App\User::where('id',$element->user_id)->first();

                                        @endphp
                                         <h3 style='text-transform: none;'>{{  $userData->email  }}</h3>
                                         <p>Ref#: {{ $userData->user_ref }}</p>
                                        @if (!empty($userData->profile_photo_path))
                                           <img src="{{ asset('img') }}/{{ $userData->profile_photo_path }}" alt="">

                                        @else
                                        <img src="{{ asset('img') }}/notfoundi.png" alt="">
                                        @endif
                                       ...
                                       @else
                                        ...
                                        <p>
                                           Order No#: {{ $element->order_number }} And {{ $element->coinType }} has been {{ $element->ordertype }} Order Status is <strong>Pendding Yet.</strong>
    ...
    <div class="left-side-bar">
        <div class="brand-logo">
            <a href="{{url('dashboard')}}">
                ...
                <h3 style='color: #BC873C;'>MOUNTCOINS</h3>
                    ...
                    <li>
                        <a href="{{ route('get_all_payments_orders_data') }}" class="dropdown-toggle no-arrow">
                            <span class="mtext">Orders Sell Coins</span>
                              @php
                            $odcount=\App\Recived_Orders::where('ordertype','sell')->where('view_admin','no')->count();
                           @endphp
                           @if ($odcount>0)
                               &nbsp;&nbsp;<span class="badge badge-danger"style="background: #e23737;">New ({{$odcount  }})</span>
                           @endif
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="{{ route('get_all_payments_orders_data_buy') }}" class="dropdown-toggle no-arrow">
                            <span class="mtext">Orders Buy Coins</span>
                              @php
                            $odcount2=\App\Recived_Orders::where('ordertype','buy')->where('view_admin','no')->count();
                           @endphp
                           @if ($odcount2>0)
                               &nbsp;&nbsp;<span class="badge badge-danger"style="background: #e23737;">New ({{$odcount2  }})</span>
                           @endif
                        </a>
                    </li>
                    ...
                    <div class="card-box height-100-p widget-style3">
                        <div class="d-flex flex-wrap">
                            <div class="widget-data">
                                <div class="weight-700 font-24 text-dark">
                                    @php
                                      echo  App\Recived_Orders::where('ordertype','sell')->count();
                                    @endphp
                                </div>
                            ...
                            <div class="widget-icon">
                                <div class="icon" data-color="#ff5b5b"><i class="icon-copy dw dw-wallet"></i></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-box height-100-p widget-style3">
                        <div class="d-flex flex-wrap">
                            <div class="widget-data">
                                <div class="weight-700 font-24 text-dark">
                                    @php
                                          echo  App\Recived_Orders::where('ordertype','buy')->count();
                                    @endphp
                                </div>
                                <div class="font-14 text-secondary weight-500">Orders Buy Coins</div>
                            </div>
                            <div class="widget-icon">
                                <div class="icon">
                                    <i class="icon-copy dw dw-wallet"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="d-flex flex-wrap">
                            <div class="widget-data">
                                <div class="weight-700 font-24 text-dark">
                                    ...
                                    @php
                                          echo  App\Review::all()->count();
                                    @endphp
                                </div>
                                <div class="font-14 text-secondary weight-500">Reviews</div>
                            </div>
                            <div class="widget-icon">
